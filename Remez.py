...
def FindPeaks(E,low,high):
    scale = high-low
    dx = scale * 10**(-10) #kinda arbitrary, code im referencing uses the mpmath toolbox to get like a kinda nicer one that uses floating point stuff to get the small timestep with like relatve error on the scale region. I should probably lean towards that
    df = lambda x: np.abs((E(x+dx)-E(x-dx)))/(2*dx)
...
def remez(f,a,b,deg,tol):
    ...
    xint = chebyshev_points(a,b,deg+1)

    steperror = 100000
    numits =0
    CheckState = False

    while CheckState == False or numits < 100:
        #Do some BS
        #set up the matrix
        bvec = f(xint)
        ...
        g=lambda x: np.dot(p,np.power(x,range(deg+1)))
        E = lambda x: f(x) - g(x)
        ...
        xpeaks[0],err = FindPeaks(E,roots[0],roots[1])
        if err == 0:
            xpeaks[0] = roots[0]
        xpeaks[-1],err = FindPeaks(E,roots[-2],roots[-1])
        if err == 0:
            xpeaks[-1] = roots[-1]

        #Uses the equiosiclation and that it is required to be true due to the matrix that we are using. 
        for j in range(1,deg):
            [xpeaks[j],trash] = FindPeaks(E,roots[j],roots[j+1])
        ...
        minpeak = min(E(xpeaks))
        maxpeak = max(E(xpeaks))
